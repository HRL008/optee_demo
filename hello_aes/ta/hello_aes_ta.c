#define STR_TRACE_USER_TA "HELLO_AES"
...
static TEE_Result aes_encrypt(uint32_t param_types,
	...
	static TEE_Result aes_decrypt(uint32_t param_types,
		TEE_Param params[4])
	{
		char* src = NULL, *tmp = NULL;
		char out[1024] = {0};
			...
			uint32_t param_types, TEE_Param params[4])
{
	...
	switch (cmd_id) {
	//case TA_HELLO_AES_CMD_INC_VALUE:
	//	return inc_value(param_types, params);
	//case TA_HELLO_AES_CMD_DEC_VALUE:
	...
	case AES_VIEW:
		return aes_decrypt(param_types, params);
	default:
		return TEE_ERROR_BAD_PARAMETERS;
	}
}
