...
//#define filePath = "/optee/optee_examples/hello_aes/test.txt"
...
int main(int argc, char *argv[])
{
	...
	TEEC_Session sess;
	TEEC_Operation op;
	TEEC_UUID uuid = TA_HELLO_AES_UUID;
	uint32_t err_origin;
	char buff[256] = {0};
	char* buff2=NULL;
...
if (strcmp(argv[1], "init") == 0){
	...
	res = TEEC_InvokeCommand(&sess, AES_INIT, &op,
				 &err_origin);
	if (res != TEEC_SUCCESS)
 ...
 res = TEEC_InvokeCommand(&sess, AES_PROTECT, &op,
				&err_origin);
 if (res != TEEC_SUCCESS)
 ...
 if(strcmp(argv[2], "decrypt") == 0){
	 printf("Invoking AES_VIEW\n");

 res = TEEC_InvokeCommand(&sess, AES_VIEW, &op,&err_origin);//TA_DECRYPT_BUFFER
}
